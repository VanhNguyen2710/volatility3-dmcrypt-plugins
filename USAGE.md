...
| `linux.cryptconfigdump` | Dumps a kernel virtual-address range, usually after a `crypt_config` address is known. |
| `linux.keyringdump` | Locates and dumps Linux kernel key objects as candidate keys. |
...
This is useful when you know a block device by its major:minor number, for example `252:0`, but need the kernel inode address of that block device.
...
Use this plugin to locate and dump Linux kernel key objects.

This is useful when looking for candidate keys stored in the kernel keyring, including possible dm-crypt volume keys.

Typical use case:

```text
dmcryptprobe reveals expected key size
keyringdump extracts kernel key objects
candidate keys are filtered by size or description
```
...
The plugin writes dumped key objects into the output directory.
...
Filter by expected key size, for example 64 bytes:

```bash
find keys -type f -size 64c -printf '%s\t%p\n'
```
...
5. Run linux.keyringdump to extract candidate kernel keys.
6. Verify any candidate key and config before trusting the result.
```

## Notes

These plugins are helper tools. They do not replace forensic reasoning.

For dm-crypt analysis, a key alone is usually not enough. You also need runtime parameters such as cipher, IV mode, data offset, sector size, and backing device.

Always verify recovered key/config pairs by decrypting a known structure, such as a filesystem superblock magic.
