...
Lists Linux block-device inodes and maps major numbers to kernel inode addresses.

linux.dmcryptprobe

Starts from a block-device inode and inspects device-mapper / dm-crypt runtime objects.

linux.cryptconfigdump

Dumps a selected kernel virtual-address range, usually after a crypt_config address is known.

linux.keyringdump

Finds a Linux kernel key by exact type/description and dumps its payload.
...
This is useful when you know a block device by major number, for example 252:0, and need the kernel inode address of that block device.
